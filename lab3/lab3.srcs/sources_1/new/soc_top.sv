...
module soc_top #(
  ...
  axi u_axi (
      ...
      .s_axi_wstrb (4'b0),   //I,4
      .s_axi_wlast (1'b0),   //I,1
      .s_axi_wvalid(1'b0),   //I,1
      .s_axi_wready(),       //O,1

      .s_axi_bid   (),     //O,4
      .s_axi_bresp (),     //O,2
      .s_axi_bvalid(),     //O,1
      .s_axi_bready(1'b0), //I,1
  ...
  always @(posedge clk) begin
    if (SIMULATION) begin
      if (~resetn) begin
        if ((cache_data_ok === 1'b1) ||
            (s_axi_arvalid === 1'b1)
        ) begin
          $display("---------------------------");
          $display("Simulator: Detect error at %d ns", $time);
          $display("data_ok and *valid signals shouldn't be 1 during reset!");
        ...
        if ($isunknown(
                ...
                cache_data_ok
            ) || (cache_data_ok && $isunknown(
                cache_rdata
            ))) || ($isunknown(
                s_axi_arvalid
            ) || (s_axi_arvalid && ($isunknown(
                s_axi_araddr
            ) || $isunknown(
                s_axi_arid
            )))) || $isunknown(
          ...
          $display("---------------------------");
          $display("Simulator: Unknown signal detected at %d ns", $time);
          $display("Your output signals:");
          $display("  cache_addr_ok: %b", cache_addr_ok);
          $display("  cache_data_ok: %b", cache_data_ok);
          $display("  cache_rdata: %h", cache_rdata);
          $display("  arid: %b", s_axi_arid);
          $display("  arvalid: %b", s_axi_arvalid);
          $display("  araddr: %h", s_axi_araddr);
          $display("  rready: %b", s_axi_rready);
          $display("Simulator: Simulation stopped. Check your CPU :-(");
          $display("---------------------------");
          $finish;
        end

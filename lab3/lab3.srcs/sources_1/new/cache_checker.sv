...
/*
    ...
    address space:
        ...
        0xXXXX0008: status register ([0]: if end, [1]: if error, [2]: if tle) (read only)
...
module cache_checker #(
  ...
  cache_trace ct (
      .clka (clk),
      .addra((cpu_req & cache_addr_ok & resetn) ? (in_ptr + 1) : in_ptr),
      .douta(addr_in),
      .clkb (clk),
      .addrb(out_ptr),
      .doutb(ans_data)
  ...
  always @(posedge clk) begin
    if (~resetn) begin
      ...
      if (tle) begin
        ...
        if (cache_data_ok) out_ptr <= out_ptr + 1;
        if (cpu_req & cache_addr_ok) in_ptr <= in_ptr + 1;
        if (cache_en) begin

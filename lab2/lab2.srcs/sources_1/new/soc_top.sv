...
module soc_top #(
  ...
  wire [31:0] data_sram_out_ram;
  wire [31:0] data_sram_out_rom;

  wire        inst_addr_valid = sel == 2'b11 ? ~|inst_sram_addr[31:9] : ~|inst_sram_addr[31:7];
  wire        data_addr_valid = sel == 2'b11 ? ~|data_sram_addr[31:11] : ~|data_sram_addr[31:9];
  ...
  cpu_data_ram data_sram_ram (
      .addra(data_sram_addr[10:2]),
      .clka (clk),
      .dina (data_sram_wdata),
      .douta(data_sram_out_ram),
      .ena  (resetn & data_sram_en & data_addr_valid),
      .wea  (resetn & data_sram_en & data_addr_valid & data_sram_wen[0])
  );

  cpu_data_rom data_sram_rom (
      .addra(data_sram_addr[8:2]),
      .clka (clk),
      .douta(data_sram_out_rom),
      .ena  (resetn & data_sram_en & data_addr_valid)
  );

  reg cached_data_sram_from_rom;
  always @(posedge clk) begin
    if (~resetn) begin
      cached_data_sram_from_rom <= 1;
    end else if (data_sram_en & ~data_sram_wen[0]) begin
      cached_data_sram_from_rom <= data_sram_addr[10:9] == 2'b00;
    end
  end

  assign data_sram_rdata = cached_data_sram_from_rom ? data_sram_out_rom : data_sram_out_ram;
    ...
    if (~resetn) begin
      ...
      if (SIMULATION) begin
        if ($isunknown(data_sram_en)) begin
          ...
          $display("data_sram_addr=%h, data_sram_wen=%b, data_sram_wdata=%h", data_sram_addr,
                   data_sram_wen, data_sram_wdata);
          $display("Simulator: Simulation stopped. Check your CPU :-(");
          $display("---------------------------");
          $finish;
        end else if (data_sram_en && data_sram_wen[0] && data_sram_addr[10:9] == 2'b00) begin
          $display("---------------------------");
          $display("Simulator: Detect error at %d ns", $time);
          $display("Data write address %h is protected! (0x000-0x1fc)", data_sram_addr);
          $display("Simulator: Simulation stopped. Check your CPU :-(");
          $display("---------------------------");
          $finish;
        end
